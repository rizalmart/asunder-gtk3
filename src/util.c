...
char* heap_copy_of(const char* source)
{
    char* copy = strdup(source);
    if (copy == NULL)
    {
    	char msg[128];
    	snprintf(msg, sizeof(msg)-1, "malloc failed for %ld bytes. Out of memory.", strlen(source) + 1);
        fatalError(msg);
    }
...
void log_disc_info(cddb_disc_t * disc, const char* head)
{
	char buf[1024];
	unsigned int max_buf = sizeof(buf) - 1;

	snprintf(buf, max_buf, "%sDisc ID: %08x", head, cddb_disc_get_discid(disc));
    debugLog(buf);

	snprintf(buf, max_buf, "%sRevision: %d", head, cddb_disc_get_revision(disc));
    debugLog(buf);

	snprintf(buf, max_buf, "%sArtist: %s", head, cddb_disc_get_artist(disc));
    debugLog(buf);

	snprintf(buf, max_buf, "%sTitle: %s", head, cddb_disc_get_title(disc));
    debugLog(buf);

    snprintf(buf, max_buf, "%sCategory: %s", head, CDDB_CATEGORY[cddb_disc_get_category(disc)]);
    debugLog(buf);

    snprintf(buf, max_buf, "%sGenre: %s", head, cddb_disc_get_genre(disc));
    debugLog(buf);

    snprintf(buf, max_buf, "%sYear: %d", head, cddb_disc_get_year(disc));
    debugLog(buf);
    ...
    {
    	snprintf(buf, max_buf, "%s%3d: %s - %s", head, track_num, cddb_track_get_artist(track), cddb_track_get_title(track));
        debugLog(buf);
    }
}
